...
const merkleTreeReducer: Reducer<StateType, ActionType> = (state, action) => {
	switch (action.type) {
		case 'ADD_BOX_DATA':
			return {
				boxDataList: [...generateBoxData({ boxDataList: action.initBoxData })],
			}
		case 'RESET_BOX_DATA':
			return {
...
export const MerkleTreeIntro = ({
	...
	const [topRadiusValue, setTopRadiusValue] = useState<number | undefined>(0)
				...
				<div className="flex flex-col gap-16 px-4 py-8 sm:p-8">
					<div className="flex items-center justify-center gap-8">
						<ArcherElement id="root">
							<div className="flex w-full max-w-[min(100%,160px)] items-center justify-center bg-slatedark-6 px-2 py-4 md:aspect-video md:p-4">
								<Badge
						...
						<div className="text-slatedark-12">
							<div>
								...
								<div className="mb-2 flex gap-4">
									<Slider
										defaultValue={[0]}
										value={[topRadiusValue ?? 0]}
										max={100}
										step={1}
										onValueChange={([prev]) => {
											setTopRadiusValue(prev)
											handleRadiusChange(prev ?? 0, bottomRadiusValue ?? 0)
										}}
									/>
								</div>
							</div>
							<div>
								...
								<div className="flex gap-4">
									<Slider
										defaultValue={[0]}
										value={[bottomRadiusValue ?? 0]}
										max={100}
										step={1}
										onValueChange={([prev]) => {
											setBottomRadiusValue(prev)
											return handleRadiusChange(topRadiusValue ?? 0, prev ?? 0)
										}}
									/>
								</div>
							</div>
